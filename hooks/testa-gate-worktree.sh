...
# O toplevel esperado vem de git rev-parse --show-toplevel, a MESMA fonte que o gate
# imprime: no runner do Actions o mktemp sai em 8.3 (RUNNER~1) e o git responde a grafia
# longa (runneradmin) — comparar com o caminho do mktemp reprovou na CI do PR #242.
...
if [ "$rc" = 2 ] && printf '%s' "$saida" | grep -q "a restauracao e da janela principal" && printf '%s' "$saida" | grep -q "checkout -- x.txt" && printf '%s' "$saida" | grep -q -F "$(git -C "$R" rev-parse --show-toplevel)"; then
...
if [ "$rc" = 2 ] && printf '%s' "$saida" | grep -q "a restauracao e da janela principal" && printf '%s' "$saida" | grep -q "restore x.txt" && printf '%s' "$saida" | grep -q -F "$(git -C "$R" rev-parse --show-toplevel)"; then
