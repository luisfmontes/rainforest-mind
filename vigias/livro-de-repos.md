...
## A pergunta 4 tem uma lacuna de critério

A pergunta 4 reprova "**com evidência de issue**, nunca impressão". Essa redação nasceu
para barrar impressão de README — e barra. Mas ela **não admite prova de código-fonte**,
que é mais dura que issue. O caso apareceu em 2026-08-12 no `fockus/claude-skill-find-skill`:
**zero issue de Windows** (o repo tem 1 issue em toda a história), e ainda assim
`cli.py:38` crava `os.execv("/bin/bash", ...)` com caminho absoluto e sem detecção de
plataforma — o pacote é inutilizável fora de WSL/Git-Bash. Pelo critério como está
escrito, isso não reprova.

Repo novo ou sem usuário de Windows não tem issue de Windows **porque ninguém tentou**,
não porque funciona. Ausência de issue não é evidência de suporte.

Fica anotado como pendência de redação, não corrigido por conta própria: mudar critério
de veredito no meio de uma rodada é reescrever a régua com a peça em cima dela.
