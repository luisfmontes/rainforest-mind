...
function main() {
  ...
  const saida = { projeto: alvo, observacoes: [], abertas: [], relatorios: [], avisos: [] };
  ...
  for (const base of [PROJETO_DIR, CODIGO_ROOT]) {
    const dir = path.join(base, 'relatorios');
    if (!fs.existsSync(dir)) continue;
    for (const f of fs.readdirSync(dir).filter((x) => x.endsWith('.md')).sort()) {
      const p = path.join(dir, f);
      if (saida.relatorios.some((r) => r.arquivo === p)) continue;
      let titulo = f;
      try {
        const primeira = fs.readFileSync(p, 'utf8').split('\n').find((l) => l.startsWith('# '));
        if (primeira) titulo = primeira.replace(/^#\s*/, '').trim();
      } catch { /* fica o nome do arquivo */ }
      saida.relatorios.push({ arquivo: path.relative(base, p), titulo });
    }
    break; // o do projeto vence; só cai no do plugin se o projeto não tiver
  }
