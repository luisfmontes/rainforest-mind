...
function backup() {
  ...
  const { backup: alvoBackup, totalBackups: arquivos } = gravarBackup(alvoFoco, dirBackup, { teto, prefixo: 'foco' });

  const original = fs.readFileSync(alvoFoco, 'utf8');
  const relato = {
    raiz, teto, foco: alvoFoco, backup: alvoBackup,
    bytes: bytes(original), totalBackups: arquivos,
  };
  ...
  console.log(`  cópias guardadas: ${arquivos}/${teto}`);
}
