...
(cd "$SBP/plano-arquivo" && git init -q && git config user.email t@t && git config user.name T)
...
# Inicializar fluxo com plano.tarefas = 6 e arquivo do plano
ETR312="node scripts/estado.cjs"
$ETR312 iniciar --slug teste-312 --titulo "Teste 312" >/dev/null
# Atualizar manualmente o estado para apontar para o arquivo do plano
node -e '
const fs = require("fs");
...
msg1=$($ETR312 marcar --slug teste-312 --estagio executar --status parcial --json "{\"carimbos\":[{\"tarefa\":8,\"hash_base\":\"abc1234\"}]}" 2>&1)
cod1=$?
if [ "$cod1" = "0" ]; then
...
msg2=$($ETR312 marcar --slug teste-312 --estagio executar --status parcial --json "{\"carimbos\":[{\"tarefa\":9,\"hash_base\":\"abc1234\"}]}" 2>&1)
cod2=$?
...
msg3=$($ETR312 marcar --slug teste-312 --estagio executar --status parcial --json "{\"carimbos\":[{\"tarefa\":7,\"hash_base\":\"abc1234\"}]}" 2>&1)
cod3=$?
