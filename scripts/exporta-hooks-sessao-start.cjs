...
/**
 * Executa de fato os comandos de SessionStart declarados em hooks.json, na
 * ordem em que aparecem. Extraída do antigo corpo de `main()` para que outros
 * scripts (scripts/orcamento.cjs) reaproveitem a EXECUÇÃO real em vez de
 * duplicá-la — reimplementar este loop em outro lugar é o jeito de os dois
 * pontos divergirem silenciosamente.
 *
 * @param {string} [raiz] Se informado, força RFM_ROOT=raiz para os hooks
 *   (uso dos testes: raiz isolada, sandbox descartável). Se omitido, os hooks
  ...
  for (const grupo of sessionStartHooks) {
    if (grupo.hooks && Array.isArray(grupo.hooks)) {
      for (const hook of grupo.hooks) {
        ...
        try {
          // RFM_ROOT so entra no env quando `raiz` foi passada explicitamente.
          // Forçar sempre (mesmo com undefined) mudaria a cadeia de resolução
          // de hooks/lib/raiz.cjs para quem chama sem sandbox — nível 1
          // (RFM_ROOT) venceria níveis reais (projeto, ~/.rainforest) que uma
          // sessão de verdade usaria.
          const env = raiz ? { ...process.env, RFM_ROOT: raiz } : { ...process.env };
...
function main() {
  ...
  try {
    const { outputs, hookIndex } = executarHooksSessionStart(raiz);
...
module.exports = { lerHooksJson, executarHooksSessionStart };
