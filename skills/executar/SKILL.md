...
**Runtime do agente:** a primeira linha do briefing pode ser `Runtime: codex` para despachar via Codex CLI, ou `Runtime: claude` (default). O `conferir-entrega.cjs` não muda com o runtime — ele confere o worktree real independentemente de qual host correu o agente.
