...
| 12 | Entrega se valida na saída real | Agente reporta intenção, não resultado: critério de sucesso vem pronto no briefing, mutação reverte o comportamento real, e a validação é executar o artefato e olhar a saída — suíte verde não é evidência. Relatório **analítico** não tem artefato: confere duas citações na fonte e uma contagem declarada. E saída verde de **ferramenta** também não é evidência: confere o artefato que roda, não a mensagem de sucesso. ✅ sem comando e saída colados = não verificado. Recomendação **destrutiva** de agente é hipótese: confere a cadeia causal e leva ao Luís, nunca roda direto |
