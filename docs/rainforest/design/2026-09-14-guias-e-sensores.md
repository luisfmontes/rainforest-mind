...
- **D18 — O sensor externo de D6 e o sensor exigido de D9/D13 chegam por linha declarada no briefing** — porquê: o padrão já existe e já é parseado — a linha `Runtime:` do briefing (`skills/executar/SKILL.md:71`) é lida pela portaria em `hooks/portaria.cjs:247`, com precedência prompt > manifesto > default. Inventar um segundo canal para a mesma classe de declaração criaria duas formas de dizer a mesma coisa.
