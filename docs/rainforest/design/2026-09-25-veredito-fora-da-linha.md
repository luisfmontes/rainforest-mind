...
- **D2 — A extração aceita negrito, sublinhado ou crase em volta da linha (`**VEREDITO: ok**`), e só isso** — porquê: a marcação em volta não deixa ambiguidade e aceitar poupa o turno do bloqueio; procurar o veredito em qualquer posição não entra — na primeira linha ele inverte a ordem análise → veredito (D2 do contrato), e no meio pode ser citação. Decidido pelo usuário em 2026-09-25.
