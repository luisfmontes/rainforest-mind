...
- **D6 — #253: visibilidade do remoto consultada por `gh`, com cache em disco.**
  ...
  **Emendada na revisão, 2026-09-15: o cache guarda só `publica`.** O arquivo `cache-visibilidade-repo.json` fica fora de repo git e nenhum gate confere escrita nele. Enquanto `privada` era aceita do disco, plantar uma entrada **desligava a trava** — e com `em` no futuro desligava para sempre, porque `Date.now() - em < TTL` é verdade para qualquer instante futuro. Medido contra o próprio commit que a introduziu (`5e43b525`), com o `gh` respondendo público o tempo todo: sem cache **2**, plantada `privada` **0**, plantada com `em` em +100 anos **0**, plantada com `em` de 8 dias atrás **2**. A mensagem deste mesmo hook diz ao subagente para não criar arquivo nem variável que desligue a trava; este era esse arquivo. O próprio docblock já argumentava por que `desconhecida` não se cacheia — é a resposta que decide se o termo vaza; `privada` é a mesma resposta pelo lado que **libera**, e o argumento vale com mais força. O custo de re-perguntar é baixo porque a visibilidade só é consultada quando **já houve achado**: em repo privado o `gh` roda uma vez por achado, não por escrita. Achada pela revisão e pela auditoria deste lote, as duas independentemente.

  **E a visibilidade passa a olhar TODOS os remotos do GitHub, não só o de destino.** A primeira versão perguntava ao `@{upstream}` e caía em `origin` quando não havia — e branch nova sem upstream é o caso comum, não o raro. Com `origin` privado e um fork público cadastrado, o gate consultava `origin`, liberava, o segredo entrava no commit, e `git push fork main` publicava. Medido contra `5e43b525`: exit **0** lá, exit **2** agora, e o `gh` consultado passa de `org/priv` para `me/pub`; com só o `origin` privado, exit **0** nos dois — a mudança não transforma todo repo privado em bloqueio. Um remoto público basta para bloquear, que é a mesma falha fechada do resto do gate. `remotoDeDestino` fica **intacta**: `paiJaPublicado` depende dela querer dizer outra coisa — só o remoto de destino conta para “já publicado”, e aceitar qualquer um reabriria a porta do espelho descartável.
...
- **D7 — #249: o laço das baterias vira `scripts/varrer-baterias.sh`, e o YAML o
  ...
  **Emendada na revisão, 2026-09-15: “sem mudar uma linha da lógica” não se sustentou, e três coisas mudaram por necessidade.** O bloco `run:` herdava `-eo pipefail` do `defaults: run: shell: bash` do Actions; um script invocado por `bash script.sh` não herda nada, e o script trazia só `set -u`. **(1) Piso por pasta.** O `ls scripts/testa-*.sh hooks/testa-*.sh` com piso único de 15 era satisfeito por `scripts/` sozinho — hoje são **89** lá e **27** em `hooks/`, então se o glob de `hooks/` quebrasse os 27 gates saiam da varredura e o placar dizia verde. Cada metade passa a ter piso próprio, e a descoberta usa `nullglob` em vez de `ls` dentro de `$( )`, onde o status se perdia. **(2) Arrays.** `for f in $baterias` e `for f in $vermelhas` sem aspas faziam word-splitting **e** globbing: `--so "<pasta>/*.sh"` rodava várias baterias e o placar dizia “bateria passou”, no singular, e caminho com espaço voltava partido na lista de vermelhas. **(3) A invocação pára em vez de adivinhar.** `--so <caminho qualquer>` rodava script arbitrário com a cara da varredura; `--so` sem valor e flag desconhecida caiam **calados** no modo completo, que da raiz do repositório são 116 baterias no lugar de uma mensagem de uso. `--so` passa a exigir nome `testa-*.sh` e arquivo existente — pelo **nome**, não pela pasta, porque a bateria desta própria varredura roda em caixa de areia com caminho absoluto. Todas medidas pela revisão deste lote.
...
- **D10 — #260: `token` sozinho deixa de ser evidência de credencial, e o achado passa a mostrar o trecho casado com o valor redigido.** A régua `credencial` de `scripts/conferir-publicacao.cjs` exige qualificador (`access_token`, `auth_token`, `refresh_token`, `api_token`, `bearer`) ou valor com cara de segredo; as demais palavras da lista continuam disparando sozinhas. Cada achado ganha `trecho`, impresso na saída de texto e no JSON, e mostrar a chave é **opt-in por régua** — **porquê:** este repositório é sobre tokenizar linha de comando (`tokens-comando.cjs`, `tokensComAspas`, `posicaoDeComando`), e `const token = toks[i]` era bloqueado com `pode_ser_falso: false`, a marca reservada para achado sem dúvida. Custou rodadas em três tarefas deste próprio fluxo. E a mensagem nomeava só a régua, nunca o trecho: um executor gastou duas rodadas culpando o nome `dist` e "certas estruturas `const ... = ...`". O opt-in existe porque a primeira versão da redação tomava o último grupo capturante como valor e mostrava o resto, o que na régua `jid-whatsapp` imprimia o número completo — uma correção de mensagem virando vazamento. Entrou a pedido do Luís em 2026-09-14, depois de a Issue ser aberta no meio do fluxo.
  ...
  **Emendada na revisão, 2026-09-15: a isenção é por declaração de variável, não por “valor com cara de segredo”.** A primeira versão leu “ou valor com cara de segredo” ao pé da letra e virou **isento por padrão**: `token` nu só acusava se o valor casasse `^[A-Za-z0-9+/=_-]{16,}$` **e** tivesse dígito. Quatro segredos de verdade passaram a sair limpos — JWT (o `.` não está na classe), PAT do GitLab sem dígito, 24 letras sem dígito, e segredo curto — todos exit 2 em `14c471ed` e exit 0 depois. Os dois ramos de prosa dentro daquele `if` devolviam `false` dos dois lados: código morto, sob um comentário que descrevia o contrário do que rodava. E, como a isenção de prosa de 2026-08-17 ficou presa dentro do ramo de `token`, as outras chaves passaram a acusar prosa: `senha: alguma coisa qualquer aqui` e `authorization: pode ser feita pelo gestor` saíam 0 na base e passaram a sair 2. A isenção correta é estreita e ancorada na **linha**, não no valor: `const`/`let`/`var` imediatamente antes da chave, **e** o valor sem aspa de abertura — literal entre aspas depois de `const` é segredo colado num arquivo de código. Isenção por padrão numa régua de credencial é o modo de falha que a #254 nomeia: o instrumento respondendo “não achei” quando o que houve foi “não procurei”. Achada pela revisão e pela auditoria deste lote, as duas independentemente.

- **D11 — a máscara de heredoc é permissiva por exceção, não por regra: quatro condições, e o padrão é analisar.** Trocar o corpo de um heredoc por espaços só acontece quando o `<<` é operador (e não texto dentro de aspas ou de comentário), o delimitador está citado, ele fecha em linha própria, e o corpo não alimenta interpretador nem arquivo executado adiante. **Porquê:** a primeira versão exigia só a última condição e abriu **onze** formas de atravessar o gate, todas medidas como exit 2 em `14c471ed` e exit 0 depois dela — oito na primeira rodada da auditoria, e mais **três** na segunda, em 2026-09-15: `cat > x.sh <<'EOF'` com o redirecionamento **antes** do `<<` (a condição 4 lia a linha a partir do `<<` e não via o alvo), e `<<'EOF'` **citado e fechado** dentro de aspas ou de comentário (a varredura de operador estava escrita e **nunca chamada** — o que segurava esses dois casos era a condição 3, que cai quando o delimitador fecha). Cada uma das quatro condições tem hoje catraca vermelha própria; a nota anterior de que a condição 3 seria redundância geométrica estava **errada**, medida só no heredoc de uma linha — no de várias linhas sem fechamento, desligar só ela leva o gate de exit 2 a exit 0 — inclusive `echo "a << b"` numa linha apagando o staging em massa da linha seguinte, e o mesmo pela ferramenta PowerShell, que não tem heredoc nenhum. No bash de verdade a primeira forma varre o arquivo de outra sessão, que é o incidente de 2026-08-09 que este gate existe para impedir. Duas causas: `indexOf("<<")` não distingue operador de texto, e corpo **não citado** expande `$(...)` e crase, logo executa — nunca foi dado. A #258 pedia isenção para o heredoc **citado**, e era só isso que cabia conceder. Achada pela auditoria de segurança deste mesmo lote, em 2026-09-14, antes de o `revisar` fechar.
