...
- **D3 — A triagem e um script deterministico (`scripts/triagem.cjs`), nao uma passada de LLM** — porquê: a triagem que separou `zupd01` de `ZXX01M99` foi `wc -l`, contagem de `function` e linha mais repetida — tres medidas, zero julgamento. Medida que script da de graca nao deve custar chamada de modelo, e script nao mente sobre o que mediu.
...
- **D9 — Na faixa de 40% a 60% de repetição a triagem devolve `indefinido`, não força um lado** — porquê: e o D4 se pagando — o script mede, a janela decide. A base real tem 14 arquivos nessa faixa (ex.: `ZXX02V01.tlpp` a 52,5% com 30 linhas/funcao, `updrepro.prw` a 52,3% com 112). Forcar um deles para "dado-como-codigo" faria alguem ler por amostragem um arquivo que e logica, e o erro so apareceria no fim do mapa.
- **D10 — O teto de um bloco e em bytes (~40.000 caracteres); a funcao ancora so define onde cortar** — porquê: o mesmo teto rende **16 funcoes** no `ZXX01M99.prw` e **4** no `zdanfe01.prw` — a funcao varia 4x de tamanho entre arquivos (mediana real 33 linhas, p90 120, p99 589 sobre 4.172 funcoes medidas). Contar funcao entregaria blocos de tamanho imprevisivel; em bytes, `ZXX01M99` da 14 blocos e `zfiscal01` tambem 14, comparaveis e despachaveis.
- **D11 — Duplicata exata e marcada por hash na triagem, e os caminhos irmaos viram apelido no mapa** — porquê: sao 36 grupos e 36 arquivos redundantes de 624. `zfiscal01.prw` e identico em `receituario/BASE` e `receituario/CLIENTE_B`, 13.650 linhas cada; sem a marca alguem mapeia 14 blocos duas vezes, e a reconferencia depois acusa divergencia entre dois mapas que descrevem o mesmo byte.

## Medicao que sustenta D9, D10 e D11

Sobre os 628 fontes `.prw`/`.tlpp`/`.prx` de `C:Microsigaerp-trabalhoinovacao` (624 legiveis), em 2026-08-22:

```
os 8 maiores                                  linhas  func  lin/func  repet%
templates/AreaA/zupd01.prw                 27992    18      1555    96.7
.../MOD_Submodulo/zupd02.prw      22907    17      1347    96.6
.../Submodulo/.../ZXX01M99.prw     13692   219        63    32.3
.../BASE/.../NFE_CTE_MDFE/zfiscal01.prw         13650    83       164    31.9
samples-smartx/UPD/UPDSMART.prw                11505    20       575    92.3
receituario/COACRIS - BASE/recagric.prw        10419    18       579    92.7
AreaA/OG_Barter/updbarte.prw               10375    19       546    92.0
...
As duas classes se separam sozinhas: a familia `UPD*`/`AreaA` fica acima de 92%, a logica fica perto de 32%.
