# Absorver do data-skills (Rootz): 16 ideias e commit por partes

Data: 2026-09-12. Origem: análise do marketplace `wildz-data` (plugin de dados
da Rootz, autor autorizou a leitura). O que entra aqui é a **ideia** por trás
...
- **D2 — Gate de mensagem de commit.** Hook `PreToolUse` sobre `git commit`
  ...
  metade que ninguém conferia — porquê: o data-skills prova por regex o
...
- **D5 — Exit 69 = "não pude verificar".** `conferir-entrega`,
  ...
  é "Codex sem cota" — porquê: o `run_gates.sh` do data-skills separa "gate
...
- **D6 — Mapa regra → trava, com bateria.** `docs/travas-mecanicas.md` ganha
  ...
  porquê: o CLAUDE.md do data-skills bania trailer e heredoc e o histórico
...
- **D7 — Teto de tamanho para toda skill.** `SKILL.md` de qualquer skill do
  ...
  data-skills impõe 500 linhas no `skill-author` e quatro skills furam (815,
...
- **D10 — `conferir-publicacao` lê o commit.** Além do disco, varre
  ...
  passo 7.9 do data-skills lê versão de `git show <sha>:<path>`, nunca do
...
- **D11 — Modelo de ameaça e plataforma declarados.** Cada `hooks/gate-*.cjs`
  ...
  do escopo declarado"; e o data-skills é macOS-only sem uma linha dizendo
...
- **D13 — Paridade ao portar.** `modo-dev`, seção "Despachar": briefing de
  ...
  Python→Node do data-skills carrega `pyJsonDumps`/`pySort`/`pySplitLines`
...
- **D14 — Legibilidade por terceiro e escalonamento em frase.** `fechar`
  ...
  `revisar` já existe) — porquê: 89% dos commits do data-skills são de uma
...
- Hook destrutivo genérico (`rm`, `DROP`, `kubectl delete`): o escopo do
  ...
  serviço.
- Conteúdo de domínio do data-skills (ClickHouse, Kafka, Airflow): é da
  Rootz, não do método.
