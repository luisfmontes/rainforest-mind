...
- **D7 — Consolidação agrupa por sessão de origem, a partir de 30 dias, em vez de lotes cronológicos de 10 com 60 dias** — porquê: sessão já é unidade de assunto; lote cronológico mistura temas sem relação num resumo só. **Emenda de 2026-09-18 (medição do plano, respondida pelo usuário):** sessão de origem só existe para 1.375 das 11.467 observações (12%) — as 10.092 importadas do claude-mem têm `origem` na forma `claude-mem:<seq>:<hash>`, com `<seq>` distinto em todas as linhas, que é um contador de importação e não um identificador de sessão. Agrupar só por sessão deixaria 88% do acervo sem síntese para sempre, que é o oposto do que este D7 quer. **Agrupamento de recurso para as sem sessão: `(projeto, dia de criada_em)`** — medido em 98 grupos, média 103, maior 521, apenas 3 grupos de 1 —, porque a importação é cronológica por projeto e um dia dentro de um projeto é o mais perto de "um assunto" que o dado permite; o teto de 30 por grupo fatia os grandes.
