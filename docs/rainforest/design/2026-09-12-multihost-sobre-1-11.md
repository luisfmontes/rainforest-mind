# Rainforest Mind multihost sobre a versão 1.12

## Objetivo

Reaplicar a integração nativa do Codex sobre o Rainforest Mind `1.12.0` sem
...
- **D2 — O inventário de skills é descoberto dinamicamente** — porquê: a `1.12.0` contém 19 skills e novas skills não devem exigir trocar uma constante; o contrato valida cada `skills/*/SKILL.md` encontrado.
...
- **D5 — A versão corrente recebe documentos e evidências próprios** — porquê: design, plano, estado e portão da piloto `1.7.0` descrevem outra árvore e outro cache; eles serão referenciados pelo commit `c71ecd01a73ab9208c981ff2d1eea5f6378434d7`, não importados como estado atual. O slug preserva `1-11` porque o fluxo começou nessa base, mas a entrega foi revalidada e reaplicada sobre `1.12.0` antes da tarefa 4.
...
- **D9 — Cachebuster é somente instrumento de desenvolvimento** — porquê: iterações locais usam `1.12.0+codex.<token>`, mas o portão final reinstala `1.12.0` e exige igualdade byte a byte entre commit e cache, inclusive do manifesto.
- **D10 — A adaptação será reaplicada como diff mínimo sobre a main 1.12** — porquê: o merge entre as árvores da piloto alcança 209 caminhos e cherry-picks em série carregam hashes, contagens e documentos da `1.7.0`; os 12 commits desta entrega foram reaplicados sobre `cf1ad7689f84428eb0b10943c0f1cf1a662b8faf` depois de confirmar zero sobreposição entre seus 16 caminhos e os 62 caminhos alterados na nova base.
...
- Promover a instalação `1.7.0` como prova da `1.12.0`: descartado porque evidência instalada não atravessa bump de versão nem mudança de bytes.
...
- Publicar release, enviar branch, abrir PR ou remover a piloto `1.7.0` antes de a nova instalação `1.12.0` passar por execução, revisão e verificação.
...
locais foram reaplicados sem conflito sobre `cf1ad7689f84428eb0b10943c0f1cf1a662b8faf`, preservando como referência o
