# Rainforest Mind multihost sobre a versão 1.21.1

## Objetivo

Reaplicar a integração nativa do Codex sobre o Rainforest Mind `1.21.1` sem
...
- **D2 — O inventário de skills é descoberto dinamicamente** — porquê: a `1.21.1` contém 19 skills e novas skills não devem exigir trocar uma constante; o contrato valida cada `skills/*/SKILL.md` encontrado.
...
- **D5 — A versão corrente recebe documentos e evidências próprios** — porquê: design, plano, estado e portão da piloto `1.7.0` descrevem outra árvore e outro cache; eles serão referenciados pelo commit `c71ecd01a73ab9208c981ff2d1eea5f6378434d7`, não importados como estado atual. O slug preserva `1-11` porque o fluxo começou nessa base, passou por `1.12.0`, `1.13.2` e `1.19.2`, e foi reancorado sobre `1.21.1` quando a `main` avançou durante a execução.
...
- **D9 — Cachebuster é somente instrumento de desenvolvimento** — porquê: iterações locais usam `1.21.1+codex.<token>`, mas o portão final deve reinstalar `1.21.1` e exigir igualdade byte a byte do payload de produto. A prova de instalação/cache 1.21.1 ainda precisa ser refeita depois da integração; a prova 1.19.2 é histórica e não vale como aceite desta base. A projeção exclui somente os sete documentos de governança desta entrega (`docs/HANDOVER-CODEX.md`, design, plano, estado, portão, mapa da fatia e `docs/rainforest/mapas/COBERTURA.md`): eles registram a prova depois que o cache existe e não são lidos pelo runtime. Exigir seus bytes criaria regressão infinita, porque cada commit da evidência tornaria a própria instalação recém-provada obsoleta.
- **D10 — A adaptação será reaplicada como diff mínimo sobre a main corrente** — porquê: o merge entre as árvores da piloto alcança 209 caminhos e cherry-picks em série carregam hashes, contagens e documentos da `1.7.0`; a entrega foi reancorada sobre `068468fb956b8d606e9af1800aaa91dd399fdeb8`, e o rebase descartou o commit de staging já incorporado upstream com conteúdo idêntico. Em 2026-09-19 a mesma decisão foi exercida sobre `2adbae270782a5a36512c28a5c2a5354ba05c73e` (`1.19.2`). Em 2026-09-22, o merge limpo `5d81ebcebf6ca0b6fd2a852cb1d227e1505b949a` reancorou a entrega em `5cdb90e768cb1ba808821d5bdcdf46f7a19fc782` (`1.21.1`): os 81 commits desde 1.19.2 não tocaram os arquivos de produto Codex, e entre as skills somente `skills/limpar/SKILL.md` mudou, fora das quatro âncoras normalizadas.
...
- Promover instalações `1.7.0`, `1.12.0`, `1.13.2` ou `1.19.2` como prova da `1.21.1`: descartado porque evidência instalada não atravessa bump de versão nem mudança de bytes.
...
- Publicar release, abrir PR, mesclar na `main` ou remover a piloto `1.7.0` antes de a nova instalação `1.21.1` passar por execução, revisão e verificação.
