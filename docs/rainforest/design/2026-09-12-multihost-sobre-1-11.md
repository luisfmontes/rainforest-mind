# Rainforest Mind multihost sobre a versão 1.19.2

## Objetivo

Reaplicar a integração nativa do Codex sobre o Rainforest Mind `1.19.2` sem
...
- **D2 — O inventário de skills é descoberto dinamicamente** — porquê: a `1.19.2` contém 19 skills e novas skills não devem exigir trocar uma constante; o contrato valida cada `skills/*/SKILL.md` encontrado.
...
- **D5 — A versão corrente recebe documentos e evidências próprios** — porquê: design, plano, estado e portão da piloto `1.7.0` descrevem outra árvore e outro cache; eles serão referenciados pelo commit `c71ecd01a73ab9208c981ff2d1eea5f6378434d7`, não importados como estado atual. O slug preserva `1-11` porque o fluxo começou nessa base, passou por `1.12.0` e `1.13.2` e foi reancorado sobre `1.19.2` quando a `main` avançou durante a execução.
...
- Promover instalações `1.7.0`, `1.12.0` ou `1.13.2` como prova da `1.19.2`: descartado porque evidência instalada não atravessa bump de versão nem mudança de bytes.
...
- Publicar release, abrir PR, mesclar na `main` ou remover a piloto `1.7.0` antes de a nova instalação `1.19.2` passar por execução, revisão e verificação.
