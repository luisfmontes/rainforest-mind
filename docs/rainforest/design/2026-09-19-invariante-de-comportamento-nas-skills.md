...
| `fechar` | **nao_deve** | `CONFIRMO fechar issue` | vocabulário prospectivo: uma versão futura do corpo do `fechar` volta a exigir frase digitada e ninguém nota. Não é a reversão de 16/09 — aquela vivia em `scripts/fechar-issue.cjs` (10 ocorrências em `be8c49eb^`; o hook tinha 0), fora do alcance do checador, que só lê o corpo da skill |
