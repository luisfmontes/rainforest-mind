...
### 10. gate-agente-em-voo avisa uma vez por conjunto em voo na sessão (emenda, Issue #298) [tipo: implementar]
atende: D13
arquivos: `hooks/gate-agente-em-voo.cjs`, `hooks/testa-gate-agente-em-voo.sh`
depende de: nenhuma
paralela: sim
mutacao:
  arquivo: `hooks/gate-agente-em-voo.cjs`
  de: `if (jaAvisado) process.exit(0);`
  para: `if (false) process.exit(0);`
  bateria: `bash hooks/testa-gate-agente-em-voo.sh`
  fixture: caso novo "mesma sessao, mesmo em_voo, turno seguinte (stop_hook_active falso) NAO barra de novo"
pronto quando: (`de:`/`para:` são trecho NOVO — `jaAvisado` não existe em `3d509c2d`.) Depois de decidir que vai barrar (em_voo não vazio, `stop_hook_active` falso), o hook calcula uma assinatura estável de `slug`, `estagio` e da lista de agentes em voo (agente+tarefa+desde, na ordem do estado) e lê `<git-dir>/rainforest-gate-agente-em-voo.json` (git-dir via `git -C <gitTop> rev-parse --git-dir`, resolvido contra `gitTop`); `jaAvisado` é verdadeiro quando o arquivo existe com o mesmo `session_id` do payload e a mesma assinatura. Se `jaAvisado`, sai 0 em silêncio; senão grava `{session_id, assinatura, em}` e barra (exit 2). Falha de leitura ou escrita do arquivo nunca derruba a sessão e nunca silencia um aviso que devia sair (erro de leitura = não avisado; erro de escrita = barra mesmo assim). Payload sem `session_id` barra como hoje. A última linha da mensagem passa a dizer que o aviso sai uma vez por conjunto de agentes em voo nesta sessão, e que registrar agente novo em `em_voo` faz o aviso voltar. Entrada real: payload Stop `{"session_id":"s1","cwd":"<repo>","hook_event_name":"Stop","stop_hook_active":false}` contra repo de fixture com fluxo em `executar` e `em_voo` com um agente → **exit 2**; o MESMO payload de novo (turno seguinte) → **exit 0**; o mesmo payload com `session_id` `s2` → **exit 2**; com `s1` depois de acrescentar um segundo agente ao `em_voo` → **exit 2**; nenhum desses casos cria ou altera arquivo rastreado (`git -C <repo> status --porcelain` igual antes e depois). Hoje o segundo payload sai exit 2 (o defeito). Provado por `bash hooks/testa-gate-agente-em-voo.sh` saindo 0 com esses quatro casos e os existentes (ajustando `session_id` onde um caso antigo dependia de barrar de novo na mesma sessão), e por `node scripts/conferir-mutacao.cjs --arquivo hooks/gate-agente-em-voo.cjs --de "if (jaAvisado) process.exit(0);" --para "if (false) process.exit(0);" --bateria "bash hooks/testa-gate-agente-em-voo.sh"` saindo 0 (`vermelho`). Superfície humana: quem lê o aviso vê o escopo verdadeiro ("uma vez por conjunto em voo nesta sessão"), não uma promessa que a próxima notificação desmente.

### 11. Versão 1.19.0 no plugin.json e no badge do README [tipo: configurar]
atende: D14
arquivos: `.claude-plugin/plugin.json`, `README.md`
depende de: nenhuma
paralela: sim
mutacao: n/a
  motivo: troca de número de versão; a falsificação mecânica é o `conferir-versao.cjs` contra a `origin/main`, não um ramo de código.
pronto quando: com a `origin/main` em 1.18.0, `node scripts/conferir-versao.cjs` sai **exit 0** dizendo que 1.19.0 supera a `origin/main`, e o badge do `README.md` mostra o mesmo número do `plugin.json` — provado por `node scripts/conferir-versao.cjs` e por `bash scripts/testa-versao.sh` saindo 0.
