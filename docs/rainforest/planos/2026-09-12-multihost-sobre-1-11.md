# Plano: adaptação multihost sobre o Rainforest Mind 1.21.1
...
Base corrente: `5cdb90e768cb1ba808821d5bdcdf46f7a19fc782` (`1.21.1`), incorporada pelo merge limpo
`5d81ebcebf6ca0b6fd2a852cb1d227e1505b949a` em 2026-09-22. A base anterior era
`2adbae270782a5a36512c28a5c2a5354ba05c73e` (`1.19.2`), reancorada em 2026-09-19 por merge da
`origin/main`; antes dela, `068468fb956b8d606e9af1800aaa91dd399fdeb8` (`1.13.2`) foi fixada
...
sobreposição registrada no portão. Entre 1.19.2 e 1.21.1, a `main` não toca nenhum arquivo de
produto da adaptação Codex; entre as skills, somente `skills/limpar/SKILL.md` mudou, fora das
quatro âncoras normalizadas, que permanecem byte a byte iguais.
...
- **CONFIRMADO:** `.claude-plugin/plugin.json` declara `1.21.1`; a base não
...
- **LACUNA:** instalação, cachebuster, export e projeção D9/D11 ainda não foram
  refeitos para 1.21.1; as saídas 1.19.2 são arquivo histórico e não satisfazem
...
- O cachebuster nunca entra no commit final, cuja versão de integração é exatamente `1.21.1`; o bump de release pertence ao estágio `fechar`, em commit próprio, depois de `verificar`.
...
mutacao:
  ...
  fixture: seção `manifesto Codex na mesma versao da fonte Claude`, esperando divergência `1.21.2` versus `1.21.1`
pronto quando: com Claude e Codex em `1.21.1`, o portão informa igualdade;
mudando somente o manifesto Codex para `1.21.2`, ele sai não zero e mostra os
dois valores — provado por `bash scripts/testa-versao.sh` no original e por
`node scripts/conferir-mutacao.cjs --arquivo .codex-plugin/plugin.json --de '"version": "1.21.1"' --para '"version": "1.21.2"' --bateria 'bash scripts/testa-versao.sh'`.
...
`1.21.1+codex.<token>` e o marketplace apontando para este worktree, reinstalar
...
voltam byte a byte a `1.21.1` — provado no portão por comandos, saídas, caminho
do cache e hashes antes/depois, sem registrar o cachebuster no diff final.

### 7. Reinstalar exatamente 1.21.1 e executar o contrato ponta a ponta [tipo: teste]
...
os manifestos exatamente em `1.21.1`, uma reinstalação produz cache
`1.21.1` em que todos os arquivos rastreados fora da lista fechada de
...
1.7, encontra design/plano/portão do fluxo iniciado na 1.11 e reancorado na 1.21.1, enxerga Gemini como adiado e
...
e `node scripts/conferir-fluxo.cjs creep --slug 2026-09-12-multihost-sobre-1-11 --base 5cdb90e768cb1ba808821d5bdcdf46f7a19fc782 --head HEAD`
terminam verdes; a projeção do cache `1.21.1` contra o HEAD, excluindo somente
...
`git diff --name-only 5cdb90e768cb1ba808821d5bdcdf46f7a19fc782...HEAD` contém somente os
