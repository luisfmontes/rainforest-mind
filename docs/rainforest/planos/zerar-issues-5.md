...
60s. A CI de 15/09 (windows-latest, node 24, run <run-id>) mediu **8173ms** —
...
pronto quando:
- entrada real: a duração já registrada em CI para este exato teste, 8173ms (run <run-id>, job <job-id>) — hoje classificada como "não foi cortado" (`8173 >= 8000` é verdadeiro), depois do conserto deixa de ser: `node -e "console.log(8173 >= 30000)"` devolve `false`, e a mesma duração reproduzida hoje (`node -e "console.log(8173 >= 8000)"`) devolve `true` — a inversão de classificação é o efeito
...
### 22. conferir-mutacao: cópia de worktree vinculado continua worktree vinculado [tipo: implementar]
atende: D5
arquivos: `scripts/conferir-mutacao.cjs`, `scripts/testa-conferir-mutacao.sh`
depende de: 20
paralela: nao
Achado do verificar (2026-09-16): `materializarGit` (tarefa 20) monta a cópia como checkout PRINCIPAL mesmo quando a raiz é worktree vinculado. Na cópia, `git rev-parse --git-dir` == `--git-common-dir` e a branch é `fluxo/zerar-issues-5`, então `estado.cjs iniciar` recusa ("o checkout principal está em ...") e `scripts/testa-conferir-fluxo.sh` sai `exit 1` só na cópia (`ENOENT ... r148.json`) — medido: `conferir-mutacao` da tarefa 4 sai 4 (`baseline NAO-VERDE`), a mesma bateria sai `ok: 70 falhou: 0` na árvore real. Tarefas 4 e 5 ficaram sem medição no verificar.
mutacao:
  arquivo: `scripts/conferir-mutacao.cjs`
  de: `const vinculado = gitDir !== commonDir;`
  para: `const vinculado = false;`
  bateria: `bash scripts/testa-conferir-mutacao.sh`
  timeout: `3600000`
  fixture: caso novo "raiz worktree vinculado em branch nao-padrao: copia tambem e vinculada"
pronto quando: com a raiz sendo um worktree vinculado em branch não-padrão (este worktree real, `fluxo/zerar-issues-5`), dentro da cópia `git rev-parse --git-dir` e `git rev-parse --git-common-dir` resolvem para caminhos DIFERENTES, ambos dentro do temporário da cópia (nenhum aponta para o `.git` real, exceto `objects/info/alternates`); uma bateria que roda `git branch x-copia` e `git commit --allow-empty` na cópia não cria `x-copia` nem move a branch no repo real; com raiz checkout principal, a cópia continua principal; e `node scripts/conferir-mutacao.cjs --raiz <este worktree> --arquivo scripts/conferir-fluxo.cjs --de "process.exit((algumSobreviveu || algumaFalhaDeMedicao) ? 1 : 0);" --para "process.exit(algumSobreviveu ? 1 : 0);" --bateria "bash scripts/testa-conferir-fluxo.sh" --timeout 3600000` sai 0 (hoje sai 4) — provado por `bash scripts/testa-conferir-mutacao.sh` com os casos novos. Superfície humana: o diretório temporário continua some ao fim, inclusive o gitdir do vinculado.
Emenda do verificar (2026-09-17): com `.git-comum`/`.git-privado` DENTRO da cópia, `bash scripts/testa-saude.sh` sai `72 ok, 2 falha(s)` só na cópia (DUP2: `conferir-duplicacao` acha 3 grupos de arquivos idênticos nos metadados; K: execução aninhada de pasta renomeada reprova B/C/D). Os metadados git da cópia vinculada moram FORA de `raizExecucao` (diretório temporário irmão, limpo junto), como num worktree real — e `node scripts/conferir-mutacao.cjs --arquivo scripts/saude.cjs --de "if (linha.decisao === 'deny') deny++;" --para "if (false) deny++;" --bateria "bash scripts/testa-saude.sh" --timeout 3600000` rodado deste worktree sai 0.
