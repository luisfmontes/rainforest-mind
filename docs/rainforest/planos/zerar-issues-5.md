...
mutacao:
  ...
  de: a nova linha, logo após `try { data = JSON.parse(input); } catch { process.exit(0); }` (hoje linha 145) e antes da Guarda 1 (`if (data.reason !== 'prompt_input_exit')`, hoje linha 148): `if (!data || typeof data !== 'object') process.exit(0);`
  para: `if (false) process.exit(0);` (preserva a linha para o `conferir-mutacao.cjs` substituir por igual, mas neutraliza a guarda — o `TypeError` de `data.reason` volta a estourar com payload `null`)
...
mutacao:
  ...
  de: dentro da nova `function checarDespachos()`, a linha que incrementa o contador de `deny`: `if (linha.decisao === 'deny') deny++;`
  para: `if (false) deny++;`
...
mutacao:
  arquivo: `scripts/saude.cjs`
  de: `ORDER BY processada_em ASC` (nova cláusula da query em `checarMemoria`, junto de `LIMIT 1`, substituindo o laço `for` por um único `.get()`)
  para: `ORDER BY processada_em DESC` (inverte para a mais nova)
  bateria: `bash scripts/testa-saude.sh`
...
mutacao:
  ...
  de: dentro de `escolher()`, a ordenação por DATA EFETIVA (mais recente primeiro) de uma cópia indexada de `entradas`, usada para decidir o CONJUNTO mantido — substituindo a única ordenação de hoje, a chamada `[...entradas].reverse()` (linha 192, confirmada por grep como ocorrência única no arquivo) — que decidia por posição física; o conjunto mantido, depois de decidido por data, é remontado preservando a ORDEM FÍSICA ORIGINAL das entradas escolhidas (não a ordem de data), para não embaralhar o arquivo.
  para: reverter para `[...entradas].reverse()` como ÚNICA base de decisão (o código de hoje, sem olhar a data)
...
mutacao:
  ...
  de: logo após `const fixo = Buffer.byteLength(cabecalho + rodape, 'utf8');` (hoje linha 1137), a nova condicional que corta o `rodape` quando `fixo` excede o teto derivado: `if (fixo > TETOS.ORCAMENTO_BYTES - TETOS.FOCO_MIN_BYTES) { /* corta blocoRodape por prioridade — dependencias primeiro, depois sessoes, igual a travarOrcamento — e recalcula fixo/rodape */ }` (o teto NÃO é uma constante literal nova e independente; é `ORCAMENTO_BYTES - FOCO_MIN_BYTES`, calculado, para nunca poder divergir por esquecimento — o mesmo tipo de erro que abriu a Issue #250)
  para: remover essa condicional inteira (o `fixo` volta a poder crescer sem limite, comportamento de hoje)
  bateria: `bash hooks/testa-contexto-sessao.sh`
