...
pronto quando: existe a função `corpoDeHeredoc(cmd, i)` em `hooks/gate-fechar-issue.cjs` que, com `cmd[i..]` começando por `<<` ou `<<-` seguido de delimitador nu ou entre aspas simples/duplas, devolve `{ fim, corpo, comando }` (índice logo após a linha do delimitador de fechamento, o texto do corpo, e o primeiro token do comando ao qual o heredoc pertence) e `null` quando não há heredoc ali; `segmentosParaGate` a chama por `const heredoc = corpoDeHeredoc(cmd, i);` (uma ocorrência), pula `corpo` quando `comando` não é interpretador e o entrega a `segmentosParaGate` recursivamente quando é (`bash`, `sh`, `zsh`, `ksh`, `dash`, `pwsh`, `powershell`, `cmd`, `eval`, `source`, `.`); casos novos em `hooks/testa-gate-fechar-issue.sh`, cada um mandando o payload ao hook por stdin: os cinco de prosa da Issue #239 saem 0 — `cat > d.md <<'EOF'` com `(x).`, com `(x). O que`, com `(x). o que`, com `(x) ; . O que` e com `Medido em 2026-09-12 (folga de 2 B). Ele sobe.`; heredoc com delimitador entre aspas duplas e nu idem (exit 0); `cat <<'EOF'` cujo corpo contém `gh issue close 12` sai 0 (o `cat` não executa); `bash <<'EOF'` cujo corpo contém `gh issue close 12` sai 2; `bash -c "gh issue close 12"` continua 2; **emenda da revisão (2026-09-13):** só o corpo é dado — o resto da linha do heredoc é comando e continua verificado: `cat <<EOF; gh issue close 12`, `cat <<EOF && gh issue close 12` e `cat <<'EOF' | gh pr create --body "closes #999"` (sem evidência) saem 2, e `gh issue close 12` na linha seguinte ao `EOF` idem; **emenda da segunda revisão (2026-09-13):** com `<<-` a linha de fechamento fecha depois de tiradas as tabulações à esquerda (como no bash) — `cat <<-EOF` com `\tcorpo`, `\tEOF` e `gh issue close 12` na linha seguinte sai 2, e `cat > d.md <<-'EOF'` com prosa `(x). Ele sobe.` indentada por TAB e `\t\tEOF` sai 0; **emenda da terceira revisão (2026-09-13):** `<<<` é here-string, não heredoc — `corpoDeHeredoc` devolve `null` e `cat <<<bar` seguido de `gh issue close 12` na linha seguinte sai 2; **emenda da quarta revisão (2026-09-13):** here-string dirigido a interpretador é script — `bash <<<'gh issue close 12'` sai 2 e `cat <<<'gh issue close 12'` sai 0; **emenda da quinta revisão (2026-09-13):** o comando que recebe o heredoc/here-string é resolvido atrás dos wrappers que repassam stdin (`posicaoDeComando` de `lib/tokens-comando.cjs`: `env`, `command`, `nohup`, `timeout N`, `sudo`, …) — `env bash <<'EOF'` com `gh issue close 12` no corpo e `timeout 5 bash <<<'gh issue close 12'` saem 2, `env cat <<'EOF'` com o mesmo corpo sai 0; e na palavra nua do here-string `\` escapa o caractere seguinte (`bash <<<gh\ issue\ close\ 12` sai 2); **emenda da sexta revisão (2026-09-13):** a decisão "corpo é script" passa a ser `linhaDoHeredocTemInterpretador(cmd, i)` — a linha inteira que abre o heredoc, não só o comando dono do `<<` (que continua devolvido em `comando`): `(bash) <<'EOF'`, `{ bash; } <<'EOF'` e `cat <<'EOF' | bash` com `gh issue close 12` no corpo saem 2; `(cat) <<'EOF'`, `cat <<'EOF' > d.md` e `tee d.md <<'EOF'` com prosa saem 0; **emenda da sétima revisão (2026-09-13):** a linha do heredoc é a linha *lógica* (`\`+quebra continua: `cat <<'EOF' \` + `| bash` sai 2, `cat <<'EOF' \` + `> d.md` com prosa sai 0); e o corpo tratado como dado não vira segmento, mas o seu **texto** é varrido linha a linha pelos padrões diretos do gate (`verificarTextosDeHeredoc`: `gh issue close|create|comment`, `gh pr create|edit|merge`, sem desempacotar wrapper nem veredito de ilegível, comentário `#` fora) — por isso `cat <<'EOF'` cujo corpo contém `gh issue close 12` **sai 2** (substitui o "sai 0" acima; a `main` barrava esse caso), assim como `cat <<'EOF' > s.sh` + `bash s.sh` na linha seguinte, `while read -r l; do $l; done <<'EOF'`, `mapfile -t a <<'EOF'` + `${a[0]}` e `I=bash; $I <<'EOF'`; prosa com `$(ls)`, crase, `bash -c "$X"`, `).` ou a menção parcial `gh issue` continua 0, e `# gh issue close 12` comentado idem; **emenda da oitava revisão (2026-09-13):** o texto do here-string para sumidouro entra na mesma varredura — `cat <<<'gh issue close 12'` (com ou sem `> s.sh` + `bash s.sh`) sai 2 (substitui o "sai 0" da quarta emenda), `cat <<<'Medido (x). Ele sobe.' > d.md` sai 0; heredoc sem linha de fechamento (delimitador nunca aparece) não trava o gate e o texto restante é verificado como hoje — provado por `bash hooks/testa-gate-fechar-issue.sh` exit 0 com os casos novos `ok`, e por `node scripts/conferir-mutacao.cjs` com o bloco acima saindo 0 (`vermelho`).
