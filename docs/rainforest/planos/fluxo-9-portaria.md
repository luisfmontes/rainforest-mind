...
### 10. Emenda: o que a execução acrescentou ao plano [tipo: configurar]
atende: D4, D7, D8
arquivos: `.rainforest/portaria/amostra.json`, `.rainforest/portaria/amostra-com-isolation.json`, `.rainforest/portaria/LEIA-ME.md`, `hooks/testa-portaria-tools-bloco.cjs`, `relatorios/2026-09-01-fluxo-9-t9-verificacao-manual.md`, `README.md`, `skills/rainforest-mind/references/regra-10-portaria.md`, `hooks/testa-contexto-sessao.sh`, `scripts/testa-orcamento.sh`, `scripts/testa-conferir-fluxo.sh`, `scripts/testa-versao.sh`
depende de: 9
paralela: nao
mutacao: n/a
  motivo: emenda de escopo, não comportamento novo — cada arquivo aqui já é coberto pela catraca de mutação da tarefa que o gerou (a bateria de `tools:` pelas seis mutações da tarefa 5; a segunda amostra e o LEIA-ME pela tarefa 2; o relatório da T9 pela própria verificação manual). As quatro baterias alheias consertadas têm cada uma a sua mutação, colada no commit correspondente
pronto quando: o `conferir-fluxo` não acusa creep (`node scripts/estado.cjs marcar --estagio revisar --status ok` deixa de recusar por "arquivo(s) no diff sem tarefa correspondente"), e cada grupo abaixo tem a razão escrita:

- **Segunda amostra + LEIA-ME** (`amostra-com-isolation.json`, `LEIA-ME.md`): a tarefa 2 previa UMA amostra. O despacho simples não mostra `isolation` nem `name`, que é justamente o gancho da extensão futura de `escreve: true` — a segunda amostra foi colhida num sandbox para expor esses campos, e o LEIA-ME registra o que as duas provam.
- **`hooks/testa-portaria-tools-bloco.cjs`**: bateria que não existia no plano. Nasceu do crítico da rodada 2 da revisão (lista de bloco YAML) e cresceu de 2 para 15 casos ao longo das rodadas 3 e 5. O plano previa a checagem de `tools:` dentro das tarefas 3 e 5, não uma bateria própria — que ela mereça arquivo separado é resultado de quantos formatos o parser precisou aprender.
- **`relatorios/2026-09-01-fluxo-9-t9-verificacao-manual.md`**: a tarefa 9 mandava registrar a evidência "no registro de fechamento do fluxo". Virou relatório próprio porque a evidência inclui a razão de a T9 não poder rodar dentro do `executar` — defeito de ordenação do plano, achado na execução, que não cabia numa linha de estado.
- **`README.md`** e **`references/regra-10-portaria.md`**: a tarefa 7 declarava `SKILL.md` e `regra-10.md`. O README entrou porque a tabela de regras e a de travas mecânicas afirmam o comportamento que mudou. O `regra-10-portaria.md` nasceu de uma **partição forçada**: a elaboração passou de 11.249 B contra um teto de 10.500 B por reference, e a regra 12 já tinha sido partida pelo mesmo motivo.
- **Quatro baterias alheias** (`testa-contexto-sessao.sh`, `testa-orcamento.sh`, `testa-conferir-fluxo.sh`, `testa-versao.sh`): não são deste fluxo e estão aqui por dois motivos distintos, os dois escritos no commit de cada uma. As duas primeiras ficaram **vermelhas por causa deste trabalho** (o texto novo da regra 10 estourou a catraca de núcleo e mudou o agregado medido) — dívida de merge, conserto obrigatório. As duas últimas já estavam vermelhas **na `main`**, com defeito no próprio teste, e entraram porque estavam no caminho: o `testa-conferir-fluxo` guarda a porta pela qual este `executar` fecha, e o `testa-versao` deixaria o CI deste PR vermelho por motivo alheio.
