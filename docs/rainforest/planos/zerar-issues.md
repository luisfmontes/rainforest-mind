...
pronto quando: nos dois casos de timeout (`RFM_FERRAMENTAS_TIMEOUT_MS=1` e `=abc`) a linha `EXIT=$?` vem IMEDIATAMENTE após a linha `SAIDA=$(printf ... | node "$HOOK" 2>&1)` — nenhum comando entre as duas, `unset` só depois — e a linha morta `SAIDA=$(RFM_FERRAMENTAS_TIMEOUT_MS=1 bash -c ...)` e a `UNSET_TIMEOUT=$(unset ...)` saem; fora das duas seções de TIMEOUT a bateria exporta `RFM_FERRAMENTAS_TIMEOUT_MS=15000` (os casos medem o contrato do hook, não a velocidade da máquina — emenda da CI do PR #242); com o hook íntegro a bateria sai 0, e com o hook saindo 1 no ramo incerto (a mutação acima) a asserção `ok exit 0 (D10)` do caso TIMEOUT 1 vira FALHA e a bateria sai ≠ 0 — provado por `bash hooks/testa-ferramentas-consulta.sh` exit 0 e por `node scripts/conferir-mutacao.cjs` com o bloco acima saindo 0 (`vermelho`).
