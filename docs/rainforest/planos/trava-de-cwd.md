...
atende: D4, D5, D6, D8
arquivos: `hooks/gate-repo-alheio.cjs` (novo), `hooks/hooks.json`, `hooks/lib/config.cjs`, `hooks/testa-gate-repo-alheio.sh` (novo), `hooks/testa-config.sh`, `README.md`
depende de: nenhuma
paralela: sim
...
nota (emenda de 2026-08-26, apos o `creep` recusar): `hooks/testa-config.sh` entrou na lista porque registrar duas chaves novas em `CHAVES` muda a contagem que aquela bateria confere — e o `README.md` entrou porque a tabela de travas mecanicas e a contagem de casos das baterias vivem la, e README acompanha entrega. Nenhum dos dois estava previsto; o checador de creep pegou, que e para isso que ele existe. Depois do `revisar`, os consertos de submodulo e de caminho relativo entraram nos mesmos arquivos desta tarefa.
