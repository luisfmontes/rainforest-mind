...
pronto quando: com o JSON que o harness realmente manda no stdin de um `PreToolUse` — mesma forma de `.rainforest/portaria/amostra.json`, com `tool_name` igual a `"Bash"` e o comando em `tool_input.command` —, cada uma destas nove formas é negada: `git commit --no-verify -m x`, `git commit '--no-verify' -m x`, `git commit "--no-verify" -m x`, `git commit --no-"verify" -m x`, `git commit --no-veri -m x`, `git commit --no-ver -m x`, `git commit -n -m x`, `git commit --no-gpg-sign -m x` e `git push --no-verify`; e nestas quatro formas de encadeamento o `git commit --no-verify` também é negado: `foo & git commit --no-verify -m x`, `if true; then git commit --no-verify -m x; fi`, `echo $(git commit --no-verify -m x)` e `git commit ` seguido de quebra de linha e ` --no-verify -m x`; enquanto `git commit -m x`, `git push -n` e `git commit -m "removi o --no-verify do script"` passam — provado por, para cada uma das treze primeiras, `printf '%s' "$PAYLOAD" | node hooks/gate-git-verificacao.cjs; echo $?` devolvendo `2`, e para as três últimas devolvendo `0`
