...
| `gate-publicacao-destino.cjs` | escrita de dados sensíveis (JID, telefone, email, credencial) em arquivo rastreado por git, **so quando o repositorio nao for privado**: a visibilidade do remoto e apurada por `gh repo view --json isPrivate` antes de qualquer bloqueio, com cache de 7 dias na pasta de dados. Falha FECHADA — so `isPrivate: true` libera; sem remoto, sem `gh`, `gh` com exit != 0, saida ilegivel ou `RAINFOREST_GATE_SEM_REDE=1` contam como visibilidade desconhecida e bloqueiam. O cache guarda so resposta conclusiva, para indisponibilidade de rede nao virar uma semana de decisao errada (Issue #253) | **qualquer ferramenta que escreve** (`Write`, `Edit`, `MultiEdit`) — impede vazamento em repo público |
