...
| `gate-staging-total.cjs` | `git add` com caminho total (`-A`, `--all`, `-u`, `--update`, `.`, `./`, `:/`, `*`), inclusive em flag combinada, e `git commit -a/-am/--all`. Corpo de heredoc e **dado**: antes de segmentar, o corpo de cada heredoc cujo comando receptor nao e interpretador e trocado por espacos (mascarado, nunca recortado — o comprimento tem de ser preservado para o indice do segmento continuar batendo com o de `cwdPorSegmento`), e por isso `cat > x.md <<'EOF'` com prosa passa, inclusive prosa que cita staging em massa; `bash <<'EOF'` com staging em massa dentro continua barrando, porque ali o corpo e comando (Issue #258, mesma causa da #239) | **também a janela principal**, que foi onde os dois incidentes ocorreram |
