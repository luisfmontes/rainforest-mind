...
| `gate-fechar-issue.cjs` | `gh issue close` direto e `closes #N` em PR sem comentário de evidência marcado; comando encapsulado (`bash -c`, `eval`) que o hook não lê. Corpo de heredoc é **dado**: `cat > x.md <<'EOF'` com prosa passa, e o corpo só conta quando a linha do heredoc tem um interpretador em qualquer posição (`bash <<EOF`, `cat <<EOF \| bash`, `(sh) <<EOF`, `env bash`, `eval`, `source`, `pwsh`…) | **também a janela principal** — origem: Issue #239, em que `(folga de 2 B). Ele sobe.` dentro de um heredoc virava subshell + `source` e bloqueava o comentário de evidência |
...
Cada uma tem bateria própria (`hooks/testa-gate-*.sh`, **495 casos** nas cinco
primeiras, medidos em 2026-09-13: 201 de worktree + 144 de fechar Issue + 99 de
